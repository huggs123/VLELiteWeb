...
session_start();
	...
	$first = $conn->real_escape_string($_GET['first']);
	$last = $conn->real_escape_string($_GET['last']);

	$read = "SELECT * FROM `cater_users` WHERE id = '$userid'";
...
<div class="container-fluid">
  ...
  <div class="row">
	...
	<?php
	echo"
  	<form method='POST' action='../process/processuserupdate.php?id=$userid' enctype='multipart/form-data'>
  <div class='form-group'>
    
	<div>
	<label for='formGroupExampleInput'>First Name</label>
	<input type='text' name='first' class='form-control' id='formGroupExampleInput' value='$first'>
		
	<label for='formGroupExampleInput'>Last Name</label>		
	<input type='text' name='last' class='form-control' id='formGroupExampleInput' value='$last'>
	
	<label for='formGroupExampleInput'>Change Password</label>		
	<input type='text' name='pass' class='form-control' id='formGroupExampleInput' value=''>
  ";
  ?>
  </div>
  
  <div>
  	 <label for="validationTooltip01">Edit User Level</label>
	<select class="custom-select" name='userlevel'>
 	  ...
 	  <?php 
				...
				echo"
					...
					<option  value='$id_data'>$userlevel</option>				
...
?> 
  			...
  			<div>
						<button type='Update Info' class='btn btn-outline-dark'>Update User</button>
			</div>
